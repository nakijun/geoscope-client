...
procedure OutputAllCollectedInformationInStream(out Stream: TStringList);
...
type
	...
	public
		...
		procedure OutputToStringList(const SL: TStringList);
		procedure OutputOneLineToFile(const F: Text);
		procedure OutputOneLineToStringList(const SL: TStringLIst);
	end;
...
procedure TLeak.OutputToStringList(const SL: TStringList);
begin
	SL.Add('Leak #'+IntToStr(fID)+' ');

	case Block.KindOfBlock of
		MClass:
			SL.Add('Instance of '+TObject(PChar(Block) + SizeOf(TMemoryBlocHeader)).ClassName);
		MUser:
			SL.Add('User allocated memory (GetMem)');
		MReallocedUser:
			SL.Add('Reallocated memory (ReallocMem)');
	end;

	SL.Add(#9'Size: '+IntToStr(Block.AllocatedSize));
	if fOccurences > 1 then
		SL.Add(#9+IntToStr(fOccurences)+' Occurences')
	else
		SL.Add(#9+IntToStr(fOccurences)+' Occurence');

	if fWasFieldOfAnotherObject then
		begin
			SL.Add(#9'Was field #'+IntToStr(fOtherFieldIndex)+' of an instance of '+fOwnerClassName);

			if fOtherIsDestroyed then
				SL.Add(' (destroyed)')
			else
				SL.Add(' (not destroyed)');
		end;

	SL.Add(CallStackTextualRepresentation(Block.CallerAddress,#9));
end;
...
procedure TLeak.OutputOneLineToStringList(const SL: TStringList);
begin
	case Block.KindOfBlock of
		MClass:
			SL.Add('* Instance of '+TObject(PChar(Block) + SizeOf(TMemoryBlocHeader)).ClassName);
	...
	SL.Add('Size: '+IntToStr(Block.AllocatedSize));
end;
...
begin
	...
	OutputFile.Add(OutputFileHeader);
	...
	for i := 0 to ChronogicalInfo.Count - 1 do
		TotalLeak := TotalLeak + ChronogicalInfo.Item(i).Block.AllocatedSize;
	if StoppedDueToMax then
		OutputFile.Add( 'Total leak not accurate due to MaxLeak constant reached, but at least '+IntToStr(TotalLeak)+' bytes'#13#10)
	else
		OutputFile.Add( 'Total leak: '+IntToStr(TotalLeak)+' bytes'#13#10);

	//We output the memory leaks
	OutputFile.Add(#13#10'*** MEMCHK: Blocks STILL allocated ***'#13#10);
	for i := 0 to LeaksList.Count - 1 do
		LeaksList.Item(i).OutputToStringList(OutputFile);
	OutputFile.Add('*** MEMCHK: End of allocated blocks ***'#13#10);

	//We give chronological info
	OutputFile.Add(#13#10'*** MEMCHK: Chronological leak information ***'#13#10);
	if TimeStampsCount > 0 then
		OutputFile.Add('  Time stamp: "'+TimeStamps[0]+'"');
	LastDisplayedTimeStamp := 0;
	for i := ChronogicalInfo.Count - 1 downto 0 do
		begin
			if (TimeStampsCount > 0) and (ChronogicalInfo.Item(i).Block.LastTimeStamp > LastDisplayedTimeStamp) then
				begin
					for j := LastDisplayedTimeStamp + 1 to ChronogicalInfo.Item(i).Block.LastTimeStamp do
						OutputFile.Add('  Time stamp: "'+TimeStamps[j]+'"');
					LastDisplayedTimeStamp := ChronogicalInfo.Item(i).Block.LastTimeStamp;
				end;
			ChronogicalInfo.Item(i).OutputOneLineToStringList(OutputFile);
		end;
	for j := LastDisplayedTimeStamp + 1 to TimeStampsCount - 1 do
		OutputFile.Add('  Time stamp: "'+TimeStamps[j]+'"');
	OutputFile.Add(#13#10'*** MEMCHK: End of chronological leak information ***'#13#10);

	//Output the allocation stats if necessary
	if CollectStatsAboutObjectAllocation then
		begin
			OutputFile.Add(#13#10'*** MEMCHK: Allocation stats ***'#13#10);
			if TotalLeak > 0 then
				OutputFile.Add(#9'The information is not accurate since there are memory leaks'#13#10);
			OutputFile.Add(#9'Nb instances'#9'Instance size'#9'ClassName');
			for i := 0 to AllocStatsCount - 1 do
				OutputFile.Add(#9+IntToStr(AllocatedInstances[i])+#9#9+IntToStr(AllocatedObjectsClasses[i].InstanceSize)+#9#9+AllocatedObjectsClasses[i].ClassName);
			OutputFile.Add(#13#10'*** MEMCHK: End of allocation stats ***'#13#10);
		end;
	if ComputeMemoryUsageStats then
		begin
			OutputFile.Add(#13#10'*** MEMCHK: Memory usage stats ***'#13#10);
			for i := 0 to MemoryUsageStatsCount - 1 do
				OutputFile.Add(#9+IntToStr(MemoryUsageStats[i]));
			OutputFile.Add(#13#10'*** MEMCHK: End of memory usage stats ***'#13#10);
		end;
	if KeepMaxMemoryUsage then
		OutputFile.Add(#13#10'*** Biggest memory usage was: '+IntToStr(MaxMemoryUsage)+' ***' + #13#10#13#10#13#10);
	...
	if CheckWipedBlocksOnTermination then
		begin
			...
			OutputFile.Add(#13#10'*** MEMCHK: Blocks written to after destruction ***'#13#10);
			if StoppedDueToMax then
				OutputFile.Add(#9'Number of bad blocks not accurate  due to MaxLeak constant reached, but at least '+IntToStr(BadBlocks.Count)+#13#10)
			else
				OutputFile.Add(#9'Bad blocks count: '+IntToStr(BadBlocks.Count)+#13#10);

			for i := 0 to BadBlocks.Count - 1 do
				begin
					OutputFile.Add(#9'* Destroyed block damaged');
					OutputFile.Add(#9#9'Call stack at allocation time:');
					OutputFile.Add(CallStackTextualRepresentation(BadBlocks.Item(i).CallerAddress, #9#9#9));
					OutputFile.Add(#9#9'Destroyed at: '+TextualDebugInfoForAddress(Cardinal(BadBlocks.Item(i).DestructionAdress)));
				end;

			OutputFile.Add(#13#10'*** MEMCHK: End of blocks written to after destruction ***'#13#10);
		end;
	BadBlocks.Destroy;
