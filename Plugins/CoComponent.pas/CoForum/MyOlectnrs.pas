...
type
  ...
  published
    ...
    procedure GetObjSize(out Width,Height: integer);
...
procedure GetObjWMF(pStream: TMemoryStream; out WMF: TMetaFile);
...
//. my method
procedure TOleContainer.GetObjSize(out Width,Height: integer);
var
  Size: TPoint;
begin
Size:=HimetricToPixels(FViewSize);
...
procedure TObjWMFGetting.Execute;
var
  Fm: TForm;
  MFC: TMetaFileCanvas;
  W,H: integer;
...
BorderStyle:=bsNone;
Parent:=Fm;
if Stream.Size = 0 then Raise Exception.Create('stream is null length'); //. =>
LoadFromStream(Stream);
GetObjSize(W,H);
Width:=W;
Height:=H;
...
procedure GetObjWMF(pStream: TMemoryStream; out WMF: TMetaFile);
begin
with TObjWMFGetting.Create(true) do
try
MF:=nil;
Stream:=pStream;
Resume;
