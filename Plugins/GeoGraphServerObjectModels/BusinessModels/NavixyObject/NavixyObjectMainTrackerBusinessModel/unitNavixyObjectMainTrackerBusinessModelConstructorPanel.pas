unit unitNavixyObjectMainTrackerBusinessModelConstructorPanel;
...
type
  TTNavixyObjectMainTrackerBusinessModelConstructorPanel = class(TNavixyObjectBusinessModelConstructorPanel)
    edGeoSpaceID: TEdit;
    ...
    Label12: TLabel;
    edDeviceConnectorServiceTariff: TEdit;
    Label13: TLabel;
    ...
    Label20: TLabel;
    edDeviceConnectorServiceKeepAliveInterval: TEdit;
    Label21: TLabel;
    edDeviceConnectorPassword: TEdit;
  ...
  private
    ...
    DeviceConnectorServiceKeepAliveInterval: word;
    DeviceConnectorPassword: string;
  public
    { Public declarations }
...
Constructor TTNavixyObjectMainTrackerBusinessModelConstructorPanel.Create(const pBusinessModelClass: TBusinessModelClass);
var
  I: integer;
begin
...
procedure TTNavixyObjectMainTrackerBusinessModelConstructorPanel.Preset(const idTVisualization,idVisualization: integer; const idHint: integer; const idUserAlert: integer; const idOnlineFlag: integer; const idLocationIsAvailableFlag: integer);
begin
Preset1(2{Yandex.Maps}, idTVisualization,idVisualization, idHint, idUserAlert, idOnlineFlag,idLocationIsAvailableFlag);
end;

procedure TTNavixyObjectMainTrackerBusinessModelConstructorPanel.Preset1(const idGeoSpace: integer; const idTVisualization,idVisualization: integer; const idHint: integer; const idUserAlert: integer; const idOnlineFlag: integer; const idLocationIsAvailableFlag: integer); 
begin
...
procedure TTNavixyObjectMainTrackerBusinessModelConstructorPanel.ValidateValues();
var
  I: integer;
begin
try
...
//.
try
DeviceConnectorServiceTariff:=StrToInt(edDeviceConnectorServiceTariff.Text);
except
  DeviceConnectorServiceTariff:=0;
  //. Raise Exception.Create('Service Tariff has a wrong number.'); //. =>
  end;
//.
try
...
//.
try
DeviceConnectorServiceKeepAliveInterval:=StrToInt(edDeviceConnectorServiceKeepAliveInterval.Text);
except
  DeviceConnectorServiceKeepAliveInterval:=120;
  //. Raise Exception.Create('wrong KeepAlive interval.'); //. =>
  end;
if (DeviceConnectorServiceKeepAliveInterval > CheckpointInterval)
 then Raise Exception.Create('KeepAlive interval less than Checkpoint interval'); //. =>
//.
DeviceConnectorPassword:=edDeviceConnectorPassword.Text;
if ((Length(DeviceConnectorPassword) < 4) OR (Length(DeviceConnectorPassword) > 10) OR (NOT TryStrToInt(DeviceConnectorPassword, {out} I)))
 then Raise Exception.Create('Wrong device password'); //. =>
end;

function TTNavixyObjectMainTrackerBusinessModelConstructorPanel.Construct(const pUserID: integer; const pUserName: WideString; const pUserPassword: WideString; const pidGeographServer: integer; const pidGeoGraphServerObject: integer): integer;
var
  ...
  ObjectBusinessModel: TNavixyObjectMainTrackerBusinessModel;
...
try
ObjectBusinessModel:=TNavixyObjectMainTrackerBusinessModel(TBusinessModel.GetModel(ObjectModel,BusinessModelClass.ID));
try
...
ObjectBusinessModel.DeviceConnectionServiceProviderTariff:=DeviceConnectorServiceTariff;
ObjectBusinessModel.DeviceConnectionServiceNumber:=DeviceConnectorServiceNumber;
ObjectBusinessModel.DeviceConnectionServiceProviderKeepAliveInterval:=DeviceConnectorServiceKeepAliveInterval;
ObjectBusinessModel.DeviceConnectionPassword:=DeviceConnectorPassword;
//.
...
procedure TTNavixyObjectMainTrackerBusinessModelConstructorPanel.sbSelectGeoSpaceClick(Sender: TObject);
var
  GeoSpaceID: integer;
  GeoSpaceName: string;
begin
