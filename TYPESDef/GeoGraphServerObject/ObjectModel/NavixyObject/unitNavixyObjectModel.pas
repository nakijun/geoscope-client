...
Type
  ...
  TNavixyObjectComponent = class;                 

  TConfigurationValue = class(TComponentValue)
  private
    ObjectComponent: TNavixyObjectComponent;
  public
  ...
  TNavixyObjectComponent = class(TSchemaComponent)
  public
  ...
  TNavixyObjectSchema = class(TComponentSchema)
  public
  ...
  TNavixyObjectDeviceComponent = class;

  TDeviceConfigurationValue = class(TComponentValue)
  private
    DeviceComponent: TNavixyObjectDeviceComponent;
  public
  ...
  TNavixyObjectDeviceComponent = class(TSchemaComponent)
  public
  ...
  TNavixyObjectDeviceSchema = class(TComponentSchema)
  public
    Constructor Create(const pObjectModel: TObjectModel);
  end;
  ...
  //. MODEL
  TNavixyObjectModel = class(TObjectModel)
  public
...
flVirtualValue:=true;
ObjectComponent:=TNavixyObjectComponent(Owner);
end;
...
{TNavixyObjectComponent}
Constructor TNavixyObjectComponent.Create(const pSchema: TComponentSchema);
begin
Inherited Create(pSchema,1,'NavixyObjectComponent');
...
{TNavixyObjectSchema}
Constructor TNavixyObjectSchema.Create(const pObjectModel: TObjectModel);
begin
Inherited Create(pObjectModel);
Name:='ObjectSchema';
RootComponent:=TNavixyObjectComponent.Create(Self);
end;
...
flVirtualValue:=true;
DeviceComponent:=TNavixyObjectDeviceComponent(Owner);
end;
...
{TNavixyObjectDeviceComponent}
Constructor TNavixyObjectDeviceComponent.Create(const pSchema: TComponentSchema);
begin
Inherited Create(pSchema,2,'NavixyObjectDeviceComponent');
//. items
//. components
...
{TNavixyObjectDeviceSchema}
Constructor TNavixyObjectDeviceSchema.Create(const pObjectModel: TObjectModel);
begin
Inherited Create(pObjectModel);
Name:='DeviceSchema';
RootComponent:=TNavixyObjectDeviceComponent.Create(Self);
end;
...
{TNavixyObjectModel}
class function TNavixyObjectModel.ID: integer;
begin
Result:=NavixyObjectModelID;
end;

class function TNavixyObjectModel.Name: string;
begin
Result:='Navixy';
end;

Constructor TNavixyObjectModel.Create(const pObjectController: TGEOGraphServerObjectController; const flFreeController: boolean);
begin
Inherited Create(pObjectController,flFreeController);
ObjectSchema:=TNavixyObjectSchema.Create(Self);
ObjectDeviceSchema:=TNavixyObjectDeviceSchema.Create(Self);
end;

function TNavixyObjectModel.GetControlPanel(): TObjectModelControlPanel;
begin
if (ControlPanel = nil)
 then ControlPanel:=TNavixyObjectControlPanel.Create(Self);
Result:=ControlPanel;
end;

function TNavixyObjectModel.IsObjectOnline: boolean;
begin
Lock.Enter;
try
Result:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).ConnectionModule.IsOnline.BoolValue;
finally
Lock.Leave;
end;
end;

function TNavixyObjectModel.ObjectVisualization: TObjectDescriptor;
begin
Lock.Enter;
try
Result:=TNavixyObjectComponent(ObjectSchema.RootComponent).Visualization.Descriptor.Value;
finally
Lock.Leave;
end;
end;

function TNavixyObjectModel.ObjectGeoSpaceID: integer;
begin
Lock.Enter;
try
Result:=TNavixyObjectComponent(ObjectSchema.RootComponent).GeoSpaceID.Value;
finally
Lock.Leave;
end;
end;

function TNavixyObjectModel.ObjectDatumID: integer;
begin
Lock.Enter;
try
Result:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.DatumID.Value;
finally
Lock.Leave;
end;
end;

procedure TNavixyObjectModel.Object_GetLocationFix(out TimeStamp: double; out DatumID: integer; out Latitude: double; out Longitude: double; out Altitude: double; out Speed: double; out Bearing: double; out Precision: double);
begin
Lock.Enter;
try
DatumID:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.DatumID.Value;
//.
TimeStamp:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.TimeStamp;
Latitude:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Latitude;
Longitude:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Longitude;
Altitude:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Altitude;
Speed:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Speed;
Bearing:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Bearing;
Precision:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Precision;
finally
Lock.Leave;
end;
end;

function TNavixyObjectModel.DeviceConnectorServiceProviderNumber: double;
begin
Lock.Enter;
try
Result:=TNavixyObjectDeviceComponent(ObjectDeviceSchema.RootComponent).ConnectionModule.ServiceProvider.Number.Value;
finally
Lock.Leave;
end;
end;

procedure TNavixyObjectModel.GetDayLogGeoData(const DayDate: TDateTime; out GeoDataStream: TMemoryStream);
Type
