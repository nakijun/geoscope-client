unit unitTMapFormatMapInstanceSelector;
...
type
  TfmTMapFormatMapInstanceSelector = class(TForm)
    Label1: TLabel;
    Image1: TImage;
    Bevel1: TBevel;
    Bevel2: TBevel;
...
Constructor TfmTMapFormatMapInstanceSelector.Create;
begin
Inherited Create(nil);
Update();
end;

procedure TfmTMapFormatMapInstanceSelector.Update;
var
  IL: TList;
  I: integer;
  idInstance: integer;
...
try
with TTMapFormatMapFunctionality.Create do
try
GetInstanceList(IL);
try
for I:=0 to IL.Count-1 do begin
  ...
  with TMapFormatMapFunctionality(TComponentFunctionality_Create(idInstance)) do
  try
  Caption:=Name;
  ImageIndex:=0;
...
function TfmTMapFormatMapInstanceSelector.Select(out idInstance: integer): boolean;
begin
...
function TfmTMapFormatMapInstanceSelector.Select(out idInstance: integer; out InstanceName: string): boolean;
begin
Caption:='Select instance (mouse double click)';
flSelected:=false;
ShowModal;
...
procedure TfmTMapFormatMapInstanceSelector.lvInstanceListDblClick(Sender: TObject);
begin
