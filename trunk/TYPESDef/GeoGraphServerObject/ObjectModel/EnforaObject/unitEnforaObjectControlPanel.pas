unit unitEnforaObjectControlPanel;
...
uses
  ...
  Buttons,
  unitUInt8BitMap, Menus;

type
  TEnforaObjectControlPanel = class(TObjectModelControlPanel)
    Panel1: TPanel;
    GroupBox1: TGroupBox;
    Label1: TLabel;
    Label2: TLabel;
  ...
  private
    ...
    Model: TEnforaObjectModel;
    ObjectRootComponent: TEnforaObjectComponent;
    DeviceRootComponent: TEnforaObjectDeviceComponent;
    fmGPIModuleValueBitmap: TfmUInt8BitMap;
    fmGPOModuleValueBitmap: TfmUInt8BitMap;

    procedure fmGPIModuleValueBitMapValueSet(const Value: byte);
    procedure fmGPIModuleValueBitMapValueSetBit(const Index: integer; const Value: boolean);
    procedure fmGPOModuleValueBitMapValueSet(const Value: byte);
    procedure fmGPOModuleValueBitMapValueSetBit(const Index: integer; const Value: boolean);
  public
    { Public declarations }

    Constructor Create(const pModel: TEnforaObjectModel);
...
Constructor TEnforaObjectControlPanel.Create(const pModel: TEnforaObjectModel);
var
  I: integer;
begin
Inherited Create(pModel);
Model:=pModel;
ObjectRootComponent:=TEnforaObjectComponent(Model.ObjectSchema.RootComponent);
DeviceRootComponent:=TEnforaObjectDeviceComponent(Model.ObjectDeviceSchema.RootComponent);
//.
...
//.
fmGPIModuleValueBitmap:=TfmUInt8BitMap.Create(Self);
...
//.
fmGPOModuleValueBitmap:=TfmUInt8BitMap.Create(Self);
...
procedure TEnforaObjectControlPanel.Update;
begin
Model.Lock.Enter;
try
...
fmGPIModuleValueBitmap.Value:=Byte(DeviceRootComponent.GPIModule.Value.Value);
//.
edGPOModuleValue.Text:=IntToStr(DeviceRootComponent.GPOModule.Value.Value);
fmGPOModuleValueBitmap.Value:=Byte(DeviceRootComponent.GPOModule.Value.Value);
//.
...
procedure TEnforaObjectControlPanel.edGeoSpaceIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edVisualizationTypeKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edVisualizationIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edHintIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edUserAlertIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edOnlineFlagIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edLocationIsAvailableFlagIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edDeviceDescriptorVendorIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edDeviceDescriptorModelIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edDeviceDescriptorSerialNumberKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edDeviceDescriptorProductionDateKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edDeviceDescriptorHWVersionKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edDeviceDescriptorSWVersionKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edConnectionModuleCheckpointIntervalKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edConnectionModuleServiceProviderIDKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edConnectionModuleServiceNumberKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.edConnectionModuleServiceProviderTariffKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key <> #$0D) then Exit; //. ->
...
procedure TEnforaObjectControlPanel.cbGPSModuleDatumIDChange(Sender: TObject);
var
  SC: TCursor;
begin
SC:=Screen.Cursor;
...
procedure TEnforaObjectControlPanel.edGPSModuleDistanceThresholdKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edGPIModuleValueKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.edGPOModuleValueKeyPress(Sender: TObject; var Key: Char);
var
  SC: TCursor;
begin
if (Key = #$0D)
...
procedure TEnforaObjectControlPanel.fmGPIModuleValueBitMapValueSet(const Value: byte);
var
  SC: TCursor;
begin
SC:=Screen.Cursor;
...
procedure TEnforaObjectControlPanel.fmGPIModuleValueBitMapValueSetBit(const Index: integer; const Value: boolean);
var
  W: byte;
  Address: TAddress;
  SC: TCursor;
...
procedure TEnforaObjectControlPanel.fmGPOModuleValueBitMapValueSet(const Value: byte);
var
  SC: TCursor;
begin
if ((NOT Model.IsObjectOnline) AND (MessageDlg('Object is offline.'#$0D#$0A'Do you want to set it on the server side ?', mtConfirmation, [mbYes,mbNo], 0) = mrYes))
...
procedure TEnforaObjectControlPanel.fmGPOModuleValueBitMapValueSetBit(const Index: integer; const Value: boolean);
var
  W: word;
  Address: TAddress;
  SC: TCursor;
...
procedure TEnforaObjectControlPanel.bbUpdatePanelClick(Sender: TObject);
begin
//. update object model
ObjectRootComponent.ReadAll();
DeviceRootComponent.ReadAll();
...
procedure TEnforaObjectControlPanel.UpdaterTimer(Sender: TObject);
begin
if (Model.IsObjectOnline())
 then begin
  lbConnectionModuleStatus.Caption:='Online';
...
procedure TEnforaObjectControlPanel.Geographserverobjecttracing1Click(Sender: TObject);
var
  SC: TCursor;
...
procedure TEnforaObjectControlPanel.Geographservercontroltracing1Click(Sender: TObject);
var
  SC: TCursor;
  LogDataStream: TMemoryStream;
  S: string;
