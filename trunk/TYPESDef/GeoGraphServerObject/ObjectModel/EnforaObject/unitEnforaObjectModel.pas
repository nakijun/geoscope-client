...
Type
  ...
  TEnforaObjectComponent = class;                 

  TConfigurationValue = class(TComponentValue)
  private
    ObjectComponent: TEnforaObjectComponent;
  public
  ...
  TEnforaObjectComponent = class(TSchemaComponent)
  public
  ...
  TEnforaObjectSchema = class(TComponentSchema)
  public
  ...
  TEnforaObjectDeviceComponent = class;

  TDeviceConfigurationValue = class(TComponentValue)
  private
    DeviceComponent: TEnforaObjectDeviceComponent;
  public
  ...
  TEnforaObjectDeviceComponent = class(TSchemaComponent)
  public
  ...
  TEnforaObjectDeviceSchema = class(TComponentSchema)
  public
    Constructor Create(const pObjectModel: TObjectModel);
  end;
  ...
  //. MODEL
  TEnforaObjectModel = class(TObjectModel)
  public
...
flVirtualValue:=true;
ObjectComponent:=TEnforaObjectComponent(Owner);
end;
...
{TEnforaObjectComponent}
Constructor TEnforaObjectComponent.Create(const pSchema: TComponentSchema);
begin
Inherited Create(pSchema,1,'EnforaObjectComponent');
...
{TEnforaObjectSchema}
Constructor TEnforaObjectSchema.Create(const pObjectModel: TObjectModel);
begin
Inherited Create(pObjectModel);
Name:='ObjectSchema';
RootComponent:=TEnforaObjectComponent.Create(Self);
end;
...
flVirtualValue:=true;
DeviceComponent:=TEnforaObjectDeviceComponent(Owner);
end;
...
{TEnforaObjectDeviceComponent}
Constructor TEnforaObjectDeviceComponent.Create(const pSchema: TComponentSchema);
begin
Inherited Create(pSchema,2,'EnforaObjectDeviceComponent');
//. items
//. components
...
BatteryModule           :=TBatteryModule.Create                 (Self,5);
DeviceDescriptor        :=TDeviceDescriptor.Create              (Self,6);
//. referencing items
...
{TEnforaObjectDeviceSchema}
Constructor TEnforaObjectDeviceSchema.Create(const pObjectModel: TObjectModel);
begin
Inherited Create(pObjectModel);
Name:='DeviceSchema';
RootComponent:=TEnforaObjectDeviceComponent.Create(Self);
end;
...
{TEnforaObjectModel}
class function TEnforaObjectModel.ID: integer;
begin
Result:=EnforaObjectModelID;
end;

class function TEnforaObjectModel.Name: string;
begin
Result:='Enfora';
end;

Constructor TEnforaObjectModel.Create(const pObjectController: TGEOGraphServerObjectController; const flFreeController: boolean);
begin
Inherited Create(pObjectController,flFreeController);
ObjectSchema:=TEnforaObjectSchema.Create(Self);
ObjectDeviceSchema:=TEnforaObjectDeviceSchema.Create(Self);
end;

function TEnforaObjectModel.GetControlPanel(): TObjectModelControlPanel;
begin
if (ControlPanel = nil)
 then ControlPanel:=TEnforaObjectControlPanel.Create(Self);
Result:=ControlPanel;
end;

function TEnforaObjectModel.IsObjectOnline: boolean;
begin
Lock.Enter;
try
Result:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).ConnectionModule.IsOnline.BoolValue;
finally
Lock.Leave;
end;
end;

function TEnforaObjectModel.ObjectVisualization: TObjectDescriptor;
begin
Lock.Enter;
try
Result:=TEnforaObjectComponent(ObjectSchema.RootComponent).Visualization.Descriptor.Value;
finally
Lock.Leave;
end;
end;

function TEnforaObjectModel.ObjectGeoSpaceID: integer;
begin
Lock.Enter;
try
Result:=TEnforaObjectComponent(ObjectSchema.RootComponent).GeoSpaceID.Value;
finally
Lock.Leave;
end;
end;

function TEnforaObjectModel.ObjectDatumID: integer;
begin
Lock.Enter;
try
Result:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.DatumID.Value;
finally
Lock.Leave;
end;
end;

procedure TEnforaObjectModel.Object_GetLocationFix(out TimeStamp: double; out DatumID: integer; out Latitude: double; out Longitude: double; out Altitude: double; out Speed: double; out Bearing: double; out Precision: double);
begin
Lock.Enter;
try
DatumID:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.DatumID.Value;
//.
TimeStamp:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.TimeStamp;
Latitude:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Latitude;
Longitude:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Longitude;
Altitude:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Altitude;
Speed:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Speed;
Bearing:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Bearing;
Precision:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).GPSModule.GPSFixData.Value.Precision;
finally
Lock.Leave;
end;
end;

function TEnforaObjectModel.DeviceConnectorServiceProviderNumber: double;
begin
Lock.Enter;
try
Result:=TEnforaObjectDeviceComponent(ObjectDeviceSchema.RootComponent).ConnectionModule.ServiceProvider.Number.Value;
finally
Lock.Leave;
end;
end;

procedure TEnforaObjectModel.GetDayLogGeoData(const DayDate: TDateTime; out GeoDataStream: TMemoryStream);
Type
