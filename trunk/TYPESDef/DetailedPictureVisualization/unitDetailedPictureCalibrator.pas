unit unitDetailedPictureCalibrator;
...
type
  TfmDetailedPictureCalibrator = class(TForm)
...
Constructor TfmDetailedPictureCalibrator.Create(const pSpace: TproxySpace; const pidDetailedImage: integer);
begin
...
Destructor TfmDetailedPictureCalibrator.Destroy;
begin
if (flParametersChanged) then memoParameters.Lines.SaveToFile(CalibrationFileName);
Inherited;
end;

procedure TfmDetailedPictureCalibrator.Google_ConvertPixXYtoLatLong(const PixX,PixY: double; const Zoom: integer; out Lat,Long: double);
begin
...
procedure TfmDetailedPictureCalibrator.Google_Calibrate;
...
var
  ...
  S: string;
  DetailedImageFunctionality: TDetailedPictureVisualizationFunctionality;
  DivX: integer;
  DivY: integer;
  SegmentWidth: double;
...
//.
DetailedImageFunctionality:=TDetailedPictureVisualizationFunctionality(TComponentFunctionality_Create(idTDetailedPictureVisualization,idDetailedImage));
...
procedure TfmDetailedPictureCalibrator.memoParametersChange(Sender: TObject);
begin
flParametersChanged:=true;
end;

procedure TfmDetailedPictureCalibrator.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Action:=caFree;
end;

procedure TfmDetailedPictureCalibrator.sbCalibrateClick(Sender: TObject);
begin
Google_Calibrate;
