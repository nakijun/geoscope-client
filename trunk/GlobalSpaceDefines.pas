...
var
  ...
  UndefinedTimestamp: double = 10000000000.0;
...
Const
  ...
  TSpaceObj_maxPointsCount = 20000;
...
Type //. SOAP support types
  ...
  function  ByteArray_ToVariant(const BA: TByteArray): Variant;
  function  ByteArray_FromVariant(const OV: Variant): TByteArray;
...
Type
  ...
  TVisualizationsHistoryItemOld = packed record
    TimeStamp: TDateTime;
    ptrObj: TPtr;
    idAct: smallint;
  end;
...
Result:=VarArrayCreate([0,Length(BA)-1],varByte);
DATAPtr:=VarArrayLock(Result);
try
Move(Pointer(@BA[0])^,DATAPtr^,Length(BA));
finally
...
DATASize:=VarArrayHighBound(OV,1)+1;
if (DATASize = 0)
 then begin
  SetLength(Result,0);
  Exit; //. ->
  end;
DATAPtr:=VarArrayLock(OV);
try
SetLength(Result,DATASize);
Move(DATAPtr^,Pointer(@Result[0])^,DATASize);
finally
VarArrayUnLock(OV);
end;
end;
