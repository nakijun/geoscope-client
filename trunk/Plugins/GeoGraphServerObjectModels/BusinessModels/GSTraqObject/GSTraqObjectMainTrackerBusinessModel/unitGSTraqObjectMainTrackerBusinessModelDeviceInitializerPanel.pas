unit unitGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel;
...
type
  TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel = class(TBusinessModelDeviceInitializerPanel)
    Timer: TTimer;
    GroupBox3: TGroupBox;
    Label7: TLabel;
    memoGSTraqDeviceHeader: TMemo;
    btnCopyGSTraqModemIDToClipboard: TBitBtn;
    GroupBox4: TGroupBox;
    PageControl2: TPageControl;
    TabSheet5: TTabSheet;
    Label10: TLabel;
    Label11: TLabel;
    bbGSTraqWriteDeviceConfiguration: TBitBtn;
    GSTraqCOMInitialization_edPortSpeed: TEdit;
    GSTraqCOMInitialization_cbPort: TComboBox;
    Label1: TLabel;
    edGSTraqDevicePassword: TEdit;
    procedure TimerTimer(Sender: TObject);
    procedure btnCopyGSTraqModemIDToClipboardClick(Sender: TObject);
    procedure bbGSTraqWriteDeviceConfigurationClick(Sender: TObject);
  private
    { Private declarations }
    BusinessModel: TGSTraqObjectMainTrackerBusinessModel;
    GeoGraphServerAddress: string;

    procedure GSTraq_InitializeByCOM;
  public
    { Public declarations }
    Constructor Create(const pBusinessModel: TGSTraqObjectMainTrackerBusinessModel);
    Destructor Destroy; override;
    procedure Update; reintroduce;
  end;

  TGSTraqControlComPort = class
  private
    Handle: THandle;
...
{TGSTraqControlComPort}
Constructor TGSTraqControlComPort.Create(const pPort: string; const pPortSpeed: integer);
var
  PortName: string;
  DCB: TDCB;
...
Destructor TGSTraqControlComPort.Destroy();
begin
if (Handle <> INVALID_HANDLE_VALUE) then CloseHandle(Handle);
Inherited;
end;

function TGSTraqControlComPort.ReadLn(): shortstring;
...
procedure TGSTraqControlComPort.WriteLn(Ln: shortstring);
var
...
procedure TGSTraqControlComPort.ProcessCommand(Command,CommandParams: string; const CommandTag: integer; const CommandPassword: string);
...
Constructor TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.Create(const pBusinessModel: TGSTraqObjectMainTrackerBusinessModel);
begin
...
Destructor TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.Destroy;
begin
Inherited;
end;

procedure TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.Update;

  function GenerateGSTraqModemHeader(const UserID: integer; const UserPassword: string; const ObjectID: integer): ShortString;
  const
    MIDLength = 16;
  var
...
//.
GSTraqCOMInitialization_cbPort.Items.BeginUpdate;
try
GSTraqCOMInitialization_cbPort.Items.Clear;
...
if (reg.OpenKey('Hardware\DeviceMap\SerialComm',False))
 then
  try
  reg.GetValueNames(GSTraqCOMInitialization_cbPort.Items);
  for I:=0 to GSTraqCOMInitialization_cbPort.Items.Count-1 do GSTraqCOMInitialization_cbPort.Items[I]:=reg.ReadString(GSTraqCOMInitialization_cbPort.Items[I]);
  finally
  reg.CloseKey;
  end;
finally
...
GSTraqCOMInitialization_cbPort.Items.EndUpdate;
end;
//.
memoGSTraqDeviceHeader.Text:=GenerateGSTraqModemHeader(ProxySpace_UserID(),ProxySpace_UserPassword(),BusinessModel.ObjectModel.ObjectController.ObjectID);
end;

procedure TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.GSTraq_InitializeByCOM;
const
  GSTraqTCPPortShift = 30;
...
ServerTCPPort:=ServerPort+GSTraqTCPPortShift;
//.
CheckpointInterval:=120;
ThresholdValue:=30;
DeviceRootComponent:=TGSTraqObjectDeviceComponent(BusinessModel.GSTraqObjectModel.ObjectDeviceSchema.RootComponent);
...
DevicePassword:=edGSTraqDevicePassword.Text;
NewDevicePassword:=DeviceRootComponent.ConnectionModule.Password.Value;
//.
with TGSTraqControlComPort.Create(GSTraqCOMInitialization_cbPort.Text,StrToInt(GSTraqCOMInitialization_edPortSpeed.Text)) do
try
CommandTag:=1;
//. set Device ID, new password
DeviceID:=BusinessModel.ObjectModel.ObjectController.ObjectID+2000000000{GSTraq correction};
...
ProcessCommand('RPHEAD','1,'+memoGSTraqDeviceHeader.Text,CommandTag,DevicePassword); Inc(CommandTag);
//. set communication 
...
//.
ShowMessage('GSTraq device has been initialized successfully');
end;

procedure TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.bbGSTraqWriteDeviceConfigurationClick(Sender: TObject);
begin
GSTraq_InitializeByCOM();
end;

procedure TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.TimerTimer(Sender: TObject);
begin
Self.BringToFront();
end;

procedure TfmGSTraqObjectMainTrackerBusinessModelDeviceInitializerPanel.btnCopyGSTraqModemIDToClipboardClick(Sender: TObject);
begin
memoGSTraqDeviceHeader.CopyToClipboard();
end;
