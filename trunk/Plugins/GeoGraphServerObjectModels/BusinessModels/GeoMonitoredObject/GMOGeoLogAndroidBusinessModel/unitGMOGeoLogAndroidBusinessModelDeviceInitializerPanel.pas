unit unitGMOGeoLogAndroidBusinessModelDeviceInitializerPanel;
...
type
  TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel = class(TBusinessModelDeviceInitializerPanel)
    Timer: TTimer;
    GroupBox1: TGroupBox;
    Panel1: TPanel;
    Panel2: TPanel;
...
Constructor TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.Create(const pBusinessModel: TGMOBusinessModel);
begin
...
Destructor TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.Destroy;
begin
Inherited;
end;

procedure TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.Update;
...
procedure TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.Initialize(const TargetFolder: string);
const
  GeoEyeConfigurationFile = 'PROFILEs\Default\GeoEye.Configuration';
  GeoEyeConfigurationFileVersion = 1;
  //.
  GeoLogConfigurationFile = 'PROFILEs\Default\GeoLog.Configuration';
  ...
  try
  ClientProgram_GetReleases(Integer(mucpModel1GeoLogAndroidClient),Localization,{out} ReleasesData);
  finally
  Release;
  end;
  //.
...
RootNode.selectSingleNode('GeoLog_QueueTransmitInterval').nodeTypedValue:=GeoLog_QueueTransmitInterval;
if (GeoLog_flSaveQueue) then RootNode.selectSingleNode('GeoLog_flSaveQueue').nodeTypedValue:='1' else RootNode.selectSingleNode('GeoLog_flSaveQueue').nodeTypedValue:='0';
RootNode.selectSingleNode('GeoLog_GPSModuleProviderReadInterval').nodeTypedValue:=GeoLog_GPSModuleProviderReadInterval;
RootNode.selectSingleNode('GeoLog_GPSModuleMapID').nodeTypedValue:=GeoLog_GPSModuleMapID;
...
if (GeoLog_flServerConnection) then RootNode.selectSingleNode('ConnectorModule_flServerConnectionEnabled').nodeTypedValue:='1' else RootNode.selectSingleNode('ConnectorModule_flServerConnectionEnabled').nodeTypedValue:='0';
RootNode.selectSingleNode('ConnectorModule_ServerAddress').nodeTypedValue:=GeoLog_ServerAddress;
RootNode.selectSingleNode('ConnectorModule_ServerPort').nodeTypedValue:=GeoLog_ServerPort;
RootNode.selectSingleNode('ConnectorModule_UserID').nodeTypedValue:=ObjectUserID;
RootNode.selectSingleNode('ConnectorModule_UserPassword').nodeTypedValue:=ObjectUserPassword;
RootNode.selectSingleNode('ConnectorModule_ObjectID').nodeTypedValue:=ObjectID;
RootNode.selectSingleNode('ConnectorModule_OSCDOQ_TransmitInterval').nodeTypedValue:=GeoLog_QueueTransmitInterval*1000;
if (GeoLog_flSaveQueue) then RootNode.selectSingleNode('ConnectorModule_OSCDOQ_flSave').nodeTypedValue:='1' else RootNode.selectSingleNode('ConnectorModule_OSCDOQ_flSave').nodeTypedValue:='0';
RootNode.selectSingleNode('ConnectorModule_LoopSleepTime').nodeTypedValue:=GeoLog_ConnectorModule_LoopSleepTime;
RootNode.selectSingleNode('GPSModule_Provider_ReadInterval').nodeTypedValue:=GeoLog_GPSModuleProviderReadInterval*1000;
RootNode.selectSingleNode('GPSModule_MapID').nodeTypedValue:=GeoLog_GPSModuleMapID;
...
procedure TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.btnUploadProgramClick(Sender: TObject);
var
  DiskPath: string;
begin
...
procedure TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.TimerTimer(Sender: TObject);
begin
Self.BringToFront();
end;

procedure TfmGMOGeoLogAndroidBusinessModelDeviceInitializerPanel.GeoLog_cbflEnabledClick(Sender: TObject);
begin
